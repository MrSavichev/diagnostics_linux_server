...
python3 diagnostics.py
       │
       ├── собирает метрики (CPU, RAM, диск, сервисы...)
       ├── автоматически передаёт их в recommendations.py
       ├── выводит полный отчёт + рекомендации в консоль
       └── сохраняет всё в один файл: report_YYYYMMDD_HHMMSS.txt
```

**Запускать нужно только один файл — `diagnostics.py`.**
`recommendations.py` вызывается им автоматически и отдельно запускать его не нужно.

> Если запустить `python3 recommendations.py` напрямую — он запустится в **тестовом режиме**
> на синтетических данных (имитация проблемного сервера), чтобы можно было посмотреть
> как выглядят рекомендации без реального сервера.

**Про несколько запусков:** каждый запуск `diagnostics.py` создаёт **отдельный файл**
с временной меткой в имени, например:
```
report_20260319_143022.txt   ← первый запуск
report_20260319_151045.txt   ← второй запуск
```
Старые файлы не перезаписываются. Рекомендации всегда относятся к **текущему** запуску —
они генерируются в реальном времени на основе только что собранных метрик, а не из файла.
...
# 4. Запустить диагностику — это единственная нужная команда
sudo python3 diagnostics.py
```

```bash
# Без sudo (часть проверок будет недоступна: lsof, LVM, auth.log)
python3 diagnostics.py

# Посмотреть как выглядят рекомендации без реального сервера (тестовый режим)
...
3. Блок рекомендаций (CRITICAL → WARN → INFO) — в том же отчёте
